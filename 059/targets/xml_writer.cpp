...
void udf::xml_writer::do_for_node(udf::for_node * const node, int lvl) {
  ...
  openTag("step", lvl + 2);
  if (node->step()) node->step()->accept(this, lvl + 4);
  closeTag("step", lvl + 2);
  openTag("block", lvl + 2);
...
void udf::xml_writer::do_tensor_dimension_node(udf::tensor_dimension_node * const node, int lvl) {
  ...
  openTag("dim_position", lvl + 2);
  node->index()->accept(this, lvl + 4);
  closeTag("dim_position", lvl + 2);
  closeTag(node, lvl);
}
...
void udf::xml_writer::do_function_definition_node(udf::function_definition_node *const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  _function = new_symbol();    // -> preciso definir no symbol
  reset_new_symbol();
